...
public class AuditedModel {
    ...
    public void save() {
        Ebean.save(this);
        audit(AuditType.CREATE,ApplicationConstants.USE_CURRENT_PRIMARY_ID);
        //System.out.println("Saving "+this.getClass().getName() + " bearing ID "+_getId());
        
        
    }
    
    private void audit(AuditType t,String idBy)
    {
    	if(Context.current().session() !=null 
        		&& Context.current().session().get(ApplicationConstants.SESSION_KEY)!=null)
        {
        	String user = Context.current().session().get(ApplicationConstants.SESSION_KEY);
        	if(idBy !=null)
        	{
        		new Audit(t, this.getClass(),user,idBy).save();
        	}else{
        		new Audit(t, this.getClass(),user,this._getId().toString()).save();
        	}
        }else{
        	// un authenticated user... this should not be happening
        	// or.. ?
        }
    }
    ...
    public void update() {
        Ebean.update(this);
        audit(AuditType.UPDATE,ApplicationConstants.USE_CURRENT_PRIMARY_ID);
    }
     ...
     */
    public void delete() {
    	String _id = _getId().toString();
    	Ebean.delete(this);
    	audit(AuditType.DELETE,_id);
    }
    ...
    public boolean equals(Object other) {
        ...
        Object id = _getId();
        Object otherId = ((AuditedModel) other)._getId();
